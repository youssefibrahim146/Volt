...
async function addHomeDeviceToUser(req, res, next) {
  try {
    ...
    validateRequiredParams({ homeDeviceId, chosenWatts }, [
      "homeDeviceId",
      "chosenWatts",
    ]);
    ...
    const actualWorkTime = systemDevice.deviceWorkAllDay
      ? 24
      : safeParseNumber(userInputWorkTime, 0);
...
async function getUserHomeDevices(req, res, next) {
  try {
    ...
    return formatResponse(
      res,
      200,
      "User home devices retrieved successfully",
      {
        devices,
        pagination: {
          page,
...
async function getUserHomeDeviceById(req, res, next) {
  try {
    ...
    return formatResponse(
      res,
      200,
      "User home device retrieved successfully",
      device
    );
  } catch (error) {
    next(error);
...
async function updateUserHomeDevice(req, res, next) {
  try {
    ...
    const watts =
      chosenWatts !== undefined ? safeParseNumber(chosenWatts) : undefined;

    if (
      watts !== undefined &&
      !existingDevice.systemDevice.wattsOptions.includes(watts)
    ) {
      const error = new Error("Invalid wattage choice");
      error.statusCode = 400;
      throw error;
    ...
    if (
      !existingDevice.systemDevice.deviceWorkAllDay &&
      userInputWorkTime !== undefined
    ) {
    ...
    return formatResponse(
      res,
      200,
      "User home device updated successfully",
      result
    );
  } catch (error) {
    next(error);
...
async function calculateUserDevicesCost(req, res, next) {
  try {
    ...
    const devicesCost = userDevices.map((device) => {
      ...
      const hours = device.systemDevice.deviceWorkAllDay
        ? 24
        : device.userInputWorkTime || 0;
...
async function getRecommendedDevices(req, res, next) {
  try {
    ...
    const remainingBudget =
      safeParseNumber(user.budget, 0) - safeParseNumber(user.minBudget, 0);
    ...
    return formatResponse(
      res,
      200,
      "Recommended devices retrieved successfully",
      {
        recommendedDevices,
        budget: {
          total: user.budget,
